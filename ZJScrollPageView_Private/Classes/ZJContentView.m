...
//    // 调整title
    ...
    if ([self.delegate respondsToSelector:@selector(ZJScrollPageViewCurrentIndex:)]) {
        [self.delegate ZJScrollPageViewCurrentIndex:_currentIndex];
    }
...
- (void)setupSubviewsWithCurrentIndex:(NSInteger)currentIndex oldIndex:(NSInteger)oldIndex {
    ...
    _currentChildVc = currentController;
